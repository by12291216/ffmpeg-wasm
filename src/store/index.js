...
const coreHistory = Symbol();
const coreUndo = Symbol();
const coreRedo = Symbol();
const coreCanUndo = Symbol();
const coreCanRedo = Symbol();
...
// 视频帧图的宽度
const VIDEO_FRAME_WIDTH = 102.44;
...
//frame是否有视频
const hasMaterial = Symbol();

//当前时间刻度的位置
const splitTimeLinePosition = Symbol();

//当前时间刻度的时间（毫秒）
const splitTime = Symbol();

//当前选择的视频索引
const currentMaterialIndex = Symbol();

//模板列表
const moduleList = Symbol();

const loadingModalVisible = Symbol();

const loadingSteps = Symbol();

const loadingStepIndex = Symbol();
...
function useProvider() {
  ...
  // const { history:$coreHistory, undo:$coreUndo, redo:$coreRedo, canUndo:$coreCanUndo, canRedo:$coreCanRedo } = useRefHistory($coreData, { capacity: 10,deep:true })
  ...
  //当前时间刻度的位置
  const $splitTimeLinePosition = ref(0);
  ...
  const $hasMaterial = computed(() => Mapping.hasVideoMaterial($coreData));
  ...
  provide(hasMaterial, $hasMaterial);
  provide(currentReadFrameVideoIndex, $currentReadFrameVideoIndex);
  provide(rightCount, $rightCount);
  provide(errorCount, $errorCount);
  provide(splitTimeLinePosition, $splitTimeLinePosition);
  provide(splitTime, $splitTime);
  provide(currentMaterialIndex, $currentMaterialIndex);
  provide(moduleList, $moduleList);
  provide(loadingModalVisible, $loadingModalVisible);
  provide(loadingSteps, $loadingSteps);
  provide(loadingStepIndex, $loadingStepIndex);
  // provide(coreHistory, $coreHistory);
  // provide(coreUndo, $coreUndo);
  // provide(coreRedo, $coreRedo);
  // provide(coreCanUndo, $coreCanUndo);
  // provide(coreCanRedo, $coreCanRedo);
}
...
const getGridFrame = (framWidth) =>
  Mapping.frameWidth2Grid(framWidth).gridFrame;
