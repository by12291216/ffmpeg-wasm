...
/**
 * 是否有可编辑的文件
 * @param {coreData}  传入coreData
 * @returns boolean
 * */
const hasVideoMaterial = (coreData) => {
  return (
...
const getCurrentTime = (
  ...
  const frame =
    (splitTimeLinePosition.value / gridWidth.value) * gridFrame.value;
  splitTime.value = frame2ms(frame, 30); //根据位置 获取时间
};

const getCurrentTimelinePlayPosition = (
  splitTimeLinePosition,
  splitTime,
  gridWidth,
  gridFrame
) => {
  splitTimeLinePosition.value =
    (ms2Frame(splitTime.value, 30) / gridFrame.value) * gridWidth.value;
};
...
const getflatFrameList = (
  ...
  for (let i = 0; i < visionTrackMaterials.length; i++) {
    ...
    for (let i = 0; i < framesNumber; i++) {
      //自修改start
      const materialIn = material.in || 0;
      const startFrame = μs2Frame(materialIn, 30); //in为自传入时间
      let frame = Math.floor(i * (videoFrameWidth / frameWidth) + startFrame);
      //自修改end
...
const constructFramesMap = (
  ...
  for (let i = 0; i < flatFrameList.length; i++) {
    ...
    if (i == 0) {
      // 修复模版第一帧错误的问题 但是这种修复也不行 后面再改
      const frames = [];
      frames.push({
        blobUrl: flatFrameList[1].blobUrl,
        frame: flatFrame.frame,
...
export default {
  hasVideoMaterial,
  getCurrentTime,
  getCurrentTimelinePlayPosition,
